...
export function generateTokensCss(
  ...
  rootLines.push("/* AUTO-GENERATED by scripts/solver-engine.ts */");
  rootLines.push(":root {");

  // Helper to format numbers
  const toNumber = (n: number) => parseFloat(n.toFixed(4));
  ...
  for (const group of groups) {
    for (const surface of group.surfaces) {
      ...
      // --- 1. Surface Token (oklch) ---
      // Pack: Lightness (L), Unused (C=0), Hue Shift (H)
      // We use oklch because H is an angle, matching Hue Shift semantics (mostly).
      const surfaceTokenName = `--surface-${surface.slug}-token`;
      propertyLines.push(`@property ${surfaceTokenName} {
  syntax: "<color>";
  inherits: true;
  initial-value: oklch(1 0 0);
      ...
      rootLines.push(
        `  ${surfaceTokenName}: light-dark(
    oklch(${toNumber(bgLight)} 0 ${toNumber(lightShift)}),
    oklch(${toNumber(bgDark)} 0 ${toNumber(darkShift)})
  );`
      );
      transitionTokens.push(surfaceTokenName);

      // --- 2. Text Token (oklab) ---
      // Pack: Text Lightness (L), Subtle Lightness (a), Subtler Lightness (b)
      // We use oklab because a/b are unitless numbers, perfect for storing Lightness values.
      const textTokenName = `--text-on-${surface.slug}-token`;
      propertyLines.push(`@property ${textTokenName} {
  syntax: "<color>";
  inherits: true;
  initial-value: oklab(0 0 0);
}`);

      rootLines.push(
        `  ${textTokenName}: light-dark(
    ...
    oklab(${toNumber(darkSpec["fg-high"])} ${toNumber(
      ...
      transitionTokens.push(textTokenName);

      // --- 3. Border Tokens (oklch with Alpha) ---
      // We need separate tokens for Decorative and Interactive borders because we need the Alpha channel.
      if (borderTargets) {
        const solveBorder = (bgL: number, textL: number, target: number) =>
        ...
        // Decorative Border Token
        const borderDecTokenName = `--border-dec-on-${surface.slug}-token`;
        propertyLines.push(`@property ${borderDecTokenName} {
  syntax: "<color>";
  inherits: true;
  initial-value: oklch(0 0 0 / 0);
        ...
        rootLines.push(
          `  ${borderDecTokenName}: light-dark(
    oklch(${toNumber(lightBorderL)} 0 0 / ${toNumber(lightDec)}),
    oklch(${toNumber(darkBorderL)} 0 0 / ${toNumber(darkDec)})
  );`
        );
        transitionTokens.push(borderDecTokenName);

        // Interactive Border Token
        const borderIntTokenName = `--border-int-on-${surface.slug}-token`;
        propertyLines.push(`@property ${borderIntTokenName} {
  syntax: "<color>";
  inherits: true;
  initial-value: oklch(0 0 0 / 0);
}`);
    ...
    oklch(${toNumber(darkBorderL)} 0 0 / ${toNumber(darkInt)})
      ...
      if (surface.states) {
        surface.states.forEach((state) => {
          ...
          if (bgState) {
            const stateTokenName = `--surface-${stateSlug}-token`;
            propertyLines.push(`@property ${stateTokenName} {
  syntax: "<color>";
  inherits: true;
  initial-value: oklch(1 0 0);
}`);
  ...
  transitionLines.push("/* Animate all tokens when color-scheme changes */");
  transitionLines.push(":root {");
  transitionLines.push(
    `  transition: ${transitionTokens
      .map((t) => `${t} 0.2s ease-in-out`)
      .join(",\n    ")};`
  );
  transitionLines.push("}");

  // Combine Property Definitions, Root Block, and Transitions
  return [...propertyLines, "", ...rootLines, ...transitionLines].join("\n");
}
