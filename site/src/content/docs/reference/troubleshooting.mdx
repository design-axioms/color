...
The `ThemeManager` must be instantiated before the DOM is rendered. If you're seeing console errors like `Cannot read property 'subscribe' of undefined`:

```tsx
// ❌ Bad: ThemeManager imported but never instantiated
import { ThemeManager } from "@axiomatic-design/color";

// ✅ Good: Initialize on app startup
import { ThemeManager } from "@axiomatic-design/color";
import { invertedSelectors } from "./theme.generated.ts";
