# RFC-TOKENS: Three-Tier Token Architecture

**Status**: Draft (standalone)  
**Date**: 2025-12-08 (original), 2026-01-05 (consolidated)  
**Source**: three-tier-tokens.md

## Summary

This RFC proposes reorganizing the Axiomatic Color system's export structure into three distinct tiers: **Primitives**, **Semantics**, and **Modes**. This aligns with the broader Design Token ecosystem (DTCG, Figma, Tokens Studio) while preserving the system's unique "Reactive Pipeline" capabilities.
...
- `config.anchors` → `color.neutral.{step}` (calculated anchor points as a scale)
...
- Example: `surface.card` is defined as a color, but its value is left abstract, to be implemented by the Modes.
...
The "Reactive Pipeline" relies on CSS variables. This reorganization suggests a naming convention update for internal CSS variables to match the tiers.
...
Update `axiomatic export` to produce a directory structure:
...
- If `--out` ends in `.json`: default to legacy single-file export (or error)
- If `--out` is a directory (or unspecified): generate multi-file structure

---

## Implementation Status

**Current State (2026-01-05)**: The system currently exports a **mode-first structure** (`light: {...}, dark: {...}`). This RFC proposes a future reorganization.

| Component         | Status             | Notes                            |
| :---------------- | :----------------- | :------------------------------- |
| Primitives tier   | ❌ Not implemented | Key colors exported flat         |
| Semantics tier    | ❌ Not implemented | Schema not separated             |
| Modes tier        | ✅ Implemented     | Current `light.json`/`dark.json` |
| Multi-file export | ❌ Not implemented | Single file only                 |

---

## Epoch Alignment

**Relationship to Epochs 44-46**: This RFC is **deferred**.

- **Alpha readiness (Epochs 45-46)** can proceed with the current mode-first export structure
- **Three-tier export** is planned for **Epoch 47 (Interoperability & Ecosystem)**
- Current export is sufficient for integration with Starlight and basic workflows

**Decision Authority**: Requires approval before implementation.

---

## Migration Path

1. **Update Exporter**: Modify `src/lib/exporters/dtcg.ts` to support 3-tier generation
2. **Update CLI**: Allow exporting to a directory
3. **Update Docs**: Explain the tiers in the "Interoperability" guide
4. **Update CSS Generator**: (Optional) Align CSS variable names with new tier terminology

---

## Related RFCs

- **RFC-CONSUMER-CONTRACT**: Token exports must align with the consumer contract
- **RFC-INTEGRATION**: Exported tokens may be consumed by integration adapters
- **RFC-CONFIGURATION**: Configuration defines what gets exported
